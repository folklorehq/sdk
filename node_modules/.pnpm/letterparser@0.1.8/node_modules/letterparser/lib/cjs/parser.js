...
exports.parseBody = exports.parseHeadersFromLines = exports.parseContentType = exports.parseHeaderValue = void 0;
const lettercoder_1 = require("lettercoder");
...
function parseHeaderValue(value) {
    ...
    if (split.length >= 2) {
        for (const parameter of split.slice(1)) {
            ...
            value = (0, helpers_js_1.unquote)(value);
            parameters[name] = (0, lettercoder_1.decodeMimeWords)(value);
        }
    }
...
function parseHeadersFromLines(lines, lineStartIdx, lineEndIdx) {
    const headers = {};
    let headerName;
    let headerValue;
    ...
    for (; lineIdx < lineEndIdx; lineIdx++) {
        ...
        if (line.startsWith(' ') || line.startsWith('\t')) {
            if (typeof headerName !== 'string' || typeof headerValue !== 'string') {
                throw new Error('Unexpected space at the beginning of line ' + (lineIdx + 1));
            }
            headerValue += '\n' + line.trim();
        }
        else {
            if (headerName && headerValue) {
                headerValue = (0, lettercoder_1.decodeMimeWords)(headerValue);
    ...
    return [headers, contentType, lineIdx];
}
exports.parseHeadersFromLines = parseHeadersFromLines;
function parseBody(depth, lines, lineStartIdx, lineEndIdx, lookaheadBoundaryLineIdx) {
    if (depth > MAX_DEPTH) {
        throw new Error('Maximum depth of ' + MAX_DEPTH + ' exceeded.');
    }
    let contents;
    let [headers, parsedType, lineIdx] = parseHeadersFromLines(lines, lineStartIdx, lineEndIdx);
    if (!parsedType) {
    ...
    if (type?.startsWith('message') && type !== 'message/delivery-status') {
        ...
        const [subcontents, newLineIdx] = parseBody(depth + 1, lines, lineIdx, endIdx);
        contents = {
        ...
        const contentsArray = [];
        if (!boundary) {
            throw new Error('Multipart type lacking boundary at line ' + (lineStartIdx + 1));
        }
        let finished = false;
        for (; lineIdx < lineEndIdx; lineIdx++) {
            const line = lines[lineIdx];
            if (line.startsWith('--' + boundary)) {
                ...
                if (!lookaheadBoundaryLineIdx) {
                    throw new Error('Multipart parsing failure (boundary lookahead failed) at line ' +
                        lineIdx);
                }
                lookaheadBoundaryLineIdx += lineIdx + 2;
                const [subcontents, newLineIdx] = parseBody(depth + 1, lines, lineIdx + 1, lineEndIdx, lookaheadBoundaryLineIdx);
                lineIdx = newLineIdx;
                if (!subcontents) {
        ...
        let body = linesSlice.join('\n');
        if (headers['Content-Transfer-Encoding']) {
            const transferEncoding = headers['Content-Transfer-Encoding'].toLowerCase();
            const stringBody = transferEncoding === 'base64' ? linesSlice.join('') : body;
            if (parsedType.type.startsWith('text/')) {
                switch (transferEncoding) {
                    case 'base64':
                        {
                            const decoder = new TextDecoder(parsedType.encoding);
                            body = decoder.decode((0, base64_js_1.toByteArray)(stringBody));
                        }
                        break;
                    case 'quoted-printable':
                        body = (0, lettercoder_1.decodeQuotedPrintable)(stringBody, parsedType.encoding);
                        break;
                }
            }
            else {
                switch (transferEncoding) {
                    case 'base64':
                        body = (0, base64_js_1.toByteArray)(stringBody);
                        break;
                    case 'quoted-printable':
                        body = (0, lettercoder_1.decodeQuotedPrintable)(stringBody);
                        break;
                }
            }
        }
