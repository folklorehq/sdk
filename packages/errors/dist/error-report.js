...
export function toErrorReport(value, opts) {
    const appError = normalizeError(value, opts.component ? { component: opts.component } : undefined);
...
function firstFrameLocation(stack) {
    if (!stack)
        return undefined;
    for (const line of stack.split('\n')) {
        if (!line.trimStart().startsWith('at ') || NODE_INTERNAL_FRAME.test(line))
            continue;
...
function fingerprint(code, component, location) {
