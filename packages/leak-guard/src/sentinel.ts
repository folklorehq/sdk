...
export function makeSentinel(seed: string): Sentinel {
...
export function sentinelNeedles(sentinel: Sentinel): string[] {
...
export function sentinelSentence(sentinel: Sentinel): string {
  const [f0, f1, f2, f3, f4] = sentinel.fragments;
  return (
    `${f0} confidential Q4 revenue and layoffs ${f1}; ` +
