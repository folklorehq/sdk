...
function unionBranches(def: ZodDefLike): unknown[] {
...
export function stringLeafPaths(schema: unknown): string[] {
  const out = new Set<string>();
  collect(schema, '', out);
  return [...out].sort();
}

function collect(schema: unknown, path: string, out: Set<string>): void {
  const def = defOf(schema);
  if (!def) return;
  const typeName = def.typeName ?? 'unknown';
  if (NO_STRING_LEAF.has(typeName)) return;
  ...
  if (!handler) {
    ...
    throw new Error(
      `zod-introspect: unhandled ZodType "${typeName}" at "${path || '<root>'}" — ` +
...
type Handler = (schema: unknown, def: ZodDefLike, path: string, out: Set<string>) => void;
...
const HANDLERS: Record<string, Handler> = {
  ...
  ZodTuple: (_s, def, path, out) =>
    (def.items ?? []).forEach((item, i) => collect(item, `${path}[${i}]`, out)),
  ...
  ZodDiscriminatedUnion: (_s, def, path, out) =>
    unionBranches(def).forEach((b) => collect(b, path, out)),
...
function collectObject(schema: unknown, path: string, out: Set<string>): void {
