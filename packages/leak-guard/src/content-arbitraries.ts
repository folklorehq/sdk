...
export function contentString(): fc.Arbitrary<string> {
  return fc.oneof(
    fc.string({ minLength: 1, maxLength: 64 }),
    fc.fullUnicodeString({ minLength: 1, maxLength: 64 }),
    fc.constantFrom(...ADVERSARIAL_FRAGMENTS),
    fc.string({ minLength: 512, maxLength: 2048 }),
    fc
...
export function contentKey(): fc.Arbitrary<string> {
...
export function contentPayload(): fc.Arbitrary<unknown> {
  const leaf = fc.oneof(contentString(), fc.integer(), fc.boolean());
  return fc.oneof(
    { withCrossShrink: true },
    leaf,
    fc.array(leaf, { maxLength: 6 }),
    fc.dictionary(fc.string({ minLength: 1, maxLength: 12 }), leaf, { maxKeys: 6 }),
    fc.array(fc.dictionary(fc.string({ minLength: 1, maxLength: 8 }), leaf, { maxKeys: 4 }), {
      maxLength: 4,
    }),
  );
}
...
export function markerEmbeddedIn(marker: string): fc.Arbitrary<string> {
  return fc
