...
import type { DirectUploadPayload, MeetingSession, VttUploadPayload } from './types.js';
...
export function normalizeMeeting(session: MeetingSession): NormalizedRecords {
...
export function normalizeDirectUpload(payload: DirectUploadPayload): NormalizedRecords {
...
export function normalizeVttUpload(payload: VttUploadPayload): NormalizedRecords {
  const segments = parseVtt(payload.vtt);
  const speakerNames = [...new Set(segments.map((s) => s.speakerId))];
  const emailByName = new Map(
    (payload.participants ?? []).map((p) => [p.displayName ?? p.id, p.email]),
  );
...
function parseVtt(vtt: string): VttCue[] {
  ...
  while (i < lines.length && !lines[i]!.includes('WEBVTT')) i++;
  i++;
