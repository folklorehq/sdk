...
const EMPTY: NormalizedRecords = { containers: [], facts: [] };

export function slackThreadId(channelId: string, threadTs: string): string {
...
function normalizeEdit(event: SlackMessage): NormalizedRecords {
  const edited = event.message;
  if (!edited?.user || !edited.ts) return EMPTY;
...
function normalizeDeletion(event: SlackMessage): NormalizedRecords {
  const deletedTs = event.deleted_ts ?? event.previous_message?.ts;
  if (!deletedTs) return EMPTY;
...
export function normalizeSlackReaction(event: SlackReactionEvent): NormalizedRecords {
  if (!event.user || !event.item?.channel || !event.item.ts) return EMPTY;
...
// the original.
export function normalizeSlackMessage(msg: SlackMessage): NormalizedRecords {
  if (msg.subtype === 'message_changed') return normalizeEdit(msg);
  if (msg.subtype === 'message_deleted') return normalizeDeletion(msg);
  if (msg.subtype && msg.subtype !== 'bot_message') return EMPTY;
  if (!msg.ts || !msg.channel || !msg.user) return EMPTY;
