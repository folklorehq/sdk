...
export function jiraIssueContainerId(issue) {
    return issue.key;
}
...
function statusCategoryToTransition(categoryKey) {
    return CATEGORY_TRANSITION[categoryKey] ?? 'updated';
}
function issueBody(issue) {
    return [issue.fields.summary, adfToText(issue.fields.description)].filter(Boolean).join('\n\n');
}
function normalizeIssueCreated(event, occurredAt) {
    const { issue } = event;
...
function normalizeIssueEdited(event, occurredAt) {
    const { issue } = event;
    const containerId = jiraIssueContainerId(issue);
    const body = issueBody(issue);
    return {
...
function normalizeIssueUpdated(event, occurredAt) {
    const statusChange = event.changelog?.items.find((item) => item.field === 'status');
    if (statusChange) {
        return normalizeStatusTransition(event, occurredAt, statusChange.fromString ?? undefined, statusChange.toString ?? event.issue.fields.status.name);
    }
    return normalizeIssueEdited(event, occurredAt);
}
export function normalizeJiraIssueEvent(event) {
...
export function normalizeJiraCommentEvent(event) {
