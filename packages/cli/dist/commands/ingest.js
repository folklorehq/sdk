...
export function corpusThemes(corpus) {
    if (corpus.themes?.length)
        return corpus.themes;
    const nested = corpus.records.themes;
    return nested ?? [];
}
export function loadCorpusFile(fixturePath) {
...
export async function runIngest(options) {
    ...
    try {
        ...
        const themes = resolvedThemes.length > 0
            ? resolvedThemes
            : [
                {
