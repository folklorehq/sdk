...
import { z } from 'zod';
import { sensitivityLevelSchema, type WikiExportTargetKind } from './enclave.js';

// The export ceiling is a one-way declassification decision: default `public`, and any
// higher tier must carry an explicit acknowledgement — the destination's own ACLs, not Folklore's
// audience model, govern access once a page leaves. Content-free label only.
export const exportCeilingSchema = sensitivityLevelSchema;
export type ExportCeiling = z.infer<typeof exportCeilingSchema>;

export const DEFAULT_EXPORT_CEILING: ExportCeiling = 'public';

// Platform-neutral projected block model. The enclave projection renders stored wiki
...
export const projectedPageSchema = z.object({
  ...
  // True only when an admin declassified above `public` (option B); logged on the target.
...
// updates the same destination page instead of duplicating.
...
// The outbound write port — the first ever. Kept separate from the inbound `Connector`
// (which "never persists anything"): this one only ever writes a projected, redacted page out.
